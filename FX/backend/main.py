"""
main.py — FX Backtest FastAPI server
Run with:  uvicorn main:app --host 0.0.0.0 --port 8001 --reload
from the fx/backend/ directory.
"""
import os
import sys
import time
...
_FX_DIR = os.path.dirname(_BACKEND_DIR)
for p in [_BACKEND_DIR, _FX_DIR]:
...
# ── App & CORS ─────────────────────────────────────────────────────────────
app = FastAPI(title="FX Backtest API", version="1.0.0")
...
async def upload_file(file: UploadFile = File(...)):
    """Save uploaded file to uploads/ and return its server-side absolute path."""
...
def backtest_run(req: BacktestRunRequest):
    ...
    try:
        ...
        # trade_log has full data (exit_date, exit_price, pnl) — use it as positions
...
def backtest_regime_optimize(req: RegimeOptimizeRequest):
    """2-D grid sweep over theta × eps as in Cell 19 of the regime notebook."""
    if not os.path.exists(req.file_path):
        _err(f"File not found: {req.file_path}")
    try:
        ...
        # Convert 2-D numpy array to list-of-lists, replacing NaN with None
        def _clean_grid(arr):
            result = []
            for row in arr:
                result.append([
...
def backtest_multi_asset(req: MultiAssetRequest):
    """Run regime backtest on multiple assets (folder or explicit file list)."""
    # Build file list
    if req.folder_path:
...
def backtest_multi_asset_scan(req: MultiAssetScanRequest):
    """Run combination test for each pair, return top N strategies per pair."""
    ...
    try:
        ...
        # Collect all unique strategy names for shared legend
        all_names = set()
