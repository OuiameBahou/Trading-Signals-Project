...
const LeadershipNetwork = () => {
    const { data: rawAllPairs, loading } = useFetch('/api/all_pairs');
    ...
    const [minMethods, setMinMethods] = useState('Any'); // 'Any', 'Triple', 'Double', 'Single'
    ...
    const { nodes, links } = useMemo(() => {
        ...
        if (minMethods === 'Triple') {
            filtered = filtered.filter(p => (p.N_Methods || 0) === 3);
        } else if (minMethods === 'Double') {
            filtered = filtered.filter(p => (p.N_Methods || 0) === 2);
        } else if (minMethods === 'Single') {
            filtered = filtered.filter(p => (p.N_Methods || 0) === 1);
        }
    ...
    return (
        <div className="h-full flex flex-col gap-4">
            ...
            <div className="t-card flex-1 relative t-border border rounded-2xl overflow-hidden min-h-0 transition-colors"
                ...
                {hoveredNode && (
                    <div
                        className="fixed pointer-events-none z-50 t-elevated t-border border rounded-xl p-3 shadow-2xl transition-opacity animate-in fade-in"
                        style={{ left: hoveredNode.x + 15, top: hoveredNode.y + 15 }}
                    >
                        <div className="flex items-center gap-2 mb-2">
                            <span className="w-2 h-2 rounded-full" style={{ background: getColor(hoveredNode.cat) }} />
                            <span className="text-xs font-black t-text tracking-widest uppercase">{hoveredNode.id}</span>
                            <span className="text-[8px] font-bold px-1.5 py-0.5 rounded-md uppercase tracking-wider border" style={{ color: getColor(hoveredNode.cat), borderColor: getColor(hoveredNode.cat) + '40', background: getColor(hoveredNode.cat) + '15' }}>{getLabel(hoveredNode.cat)}</span>
                        </div>
                        <div className="flex flex-col gap-1.5">
                            <div className="flex justify-between items-center gap-4">
                                <span className="text-[9px] font-bold t-text-m uppercase tracking-widest">Times Leader:</span>
                                <span className="text-emerald-500 font-mono font-bold text-xs bg-emerald-500/10 px-1.5 py-0.5 rounded-md border border-emerald-500/20">{hoveredNode.leaderCount}</span>
                            </div>
                            <div className="flex justify-between items-center gap-4">
                                <span className="text-[9px] font-bold t-text-m uppercase tracking-widest">Times Follower:</span>
                                <span className="text-blue-500 font-mono font-bold text-xs bg-blue-500/10 px-1.5 py-0.5 rounded-md border border-blue-500/20">{hoveredNode.followerCount}</span>
                            </div>
                        </div>
                    </div>
                )}
