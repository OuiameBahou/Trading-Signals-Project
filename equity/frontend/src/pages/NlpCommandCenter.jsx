...
import { RefreshCw, Loader2, TrendingUp, TrendingDown, Minus, AlertCircle, Newspaper, BarChart3, Brain } from 'lucide-react';
...
const HeadlineRow = ({ item }) => {
  const score = item.composite_score ?? item.finbert_score ?? null;
  return (
    <div className="flex items-start gap-4 px-4 py-3 border-b t-border-s hover:bg-white/[0.02] transition-colors last:border-b-0">
      <div className={`mt-0.5 flex-shrink-0 flex items-center gap-1 text-[9px] font-black uppercase tracking-widest ${sentColor(score)}`}>
        {score != null && sentIcon(score)}
        {sentLabel(score)}
      </div>
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 mt-1">
          ...
          {item.source && <span className="text-[9px] t-text-m">{item.source}</span>}
          <span className="text-[9px] t-text-m">{fmtDate(item.published_at ?? item.timestamp ?? item.date)}</span>
        </div>
      </div>
      {score != null && (
        <div className={`flex-shrink-0 text-[11px] font-black font-mono ${sentColor(score)}`}>
...
const NlpCommandCenter = () => {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const [sumRes, hdlRes] = await Promise.all([
        axios.get('/api/nlp/api/summary').catch(() => ({ data: null })),
        axios.get('/api/nlp/api/headlines').catch(() => ({ data: [] })),
      ]);
  ...
  /* summary stats */
  const totalNews    = summary?.total_news     ?? summary?.total_headlines ?? headlines.length ?? '—';
  const avgSent      = summary?.avg_sentiment  ?? summary?.mean_composite  ?? null;
  const bullPct      = summary?.bullish_pct    ?? summary?.pct_bullish     ?? null;
  const bearPct      = summary?.bearish_pct    ?? summary?.pct_bearish     ?? null;
  const tickerCount  = summary?.ticker_count   ?? summary?.tickers_covered ?? null;
    ...
    <div className="space-y-8">
      ...
      <div className="t-card rounded-2xl border t-border-s overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b t-border-s">
          ...
          <span className="text-[10px] t-text-m font-bold">{headlines.length} items</span>
        </div>
        {headlines.length === 0 ? (
          ...
          <div className="divide-y t-border-s max-h-[520px] overflow-y-auto custom-scrollbar">
            {headlines.slice(0, 100).map((h, i) => <HeadlineRow key={i} item={h} />)}
          </div>
        )}
      </div>

      {/* Sentiment breakdown by ticker (if in summary) */}
      {summary?.tickers && Array.isArray(summary.tickers) && summary.tickers.length > 0 && (
        <div className="t-card rounded-2xl border t-border-s overflow-hidden">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 p-5">
            {summary.tickers.map((t) => {
              const s = t.score ?? t.sentiment ?? t.composite ?? null;
              return (
                <div key={t.ticker ?? t.symbol ?? t} className="t-card border t-border-s rounded-xl p-3 text-center">
                  <div className="text-[13px] font-black font-mono t-text">{t.ticker ?? t.symbol ?? t}</div>
