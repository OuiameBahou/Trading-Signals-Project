...
/**
 * Searchable FX pair dropdown that fetches available data files from the backend.
 * Props:
 *   value    — selected pair object { name, file_path, file_type }
 *   onChange — callback receiving the selected pair object
 *   label    — optional label text (default: 'Target Pair')
 */
const PairSelector = ({ value, onChange, label = 'Target Pair' }) => {
  ...
  useEffect(() => {
    axios.get('/api/fx/data-pairs').then(r => {
      ...
      if (!value && data.length) {
        const defaultPair = data.find(p => p.name.toUpperCase().includes('EUR_USD')) || data[0];
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[12px] font-black font-mono t-text tracking-wide">
            {value ? fmtPair(value.name) : 'Select pair…'}
          </span>
        </div>
      ...
      {open && (
        <div className="absolute z-50 top-full mt-1.5 left-0 w-full t-card border t-border-s rounded-xl shadow-2xl overflow-hidden"
          ...
          <div className="overflow-y-auto custom-scrollbar" style={{ maxHeight: 230 }}>
            ...
            {filtered.length === 0 && (
              <div className="px-4 py-6 text-center text-[10px] t-text-m">No pairs match "{search}"</div>
            )}
          </div>
        </div>
      )}
