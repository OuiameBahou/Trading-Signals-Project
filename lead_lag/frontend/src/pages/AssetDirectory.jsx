...
const AssetDirectory = () => {
    const { data: rawAssets, loading, error } = useFetch('/api/assets');
    ...
    const assets = useMemo(() => {
        if (!rawAssets) return [];
        return Array.isArray(rawAssets) ? rawAssets : (rawAssets.value || []);
    }, [rawAssets]);
                ...
                <table className="w-full text-left border-collapse">
                    <thead className="text-[10px] t-text-m uppercase tracking-widest t-elevated t-border border-b font-bold transition-colors">
                        <tr>
                            ...
                            <th className="px-5 py-3 text-center">Times Leader</th>
                            <th className="px-5 py-3 text-center">Times Follower</th>
                            <th className="px-5 py-3 text-right">Total Relations</th>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <td className="px-5 py-3 text-center">
                                        <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border font-mono text-xs font-bold ${asset.Leader_Count > 0 ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-[var(--surface-hover)] border-transparent t-text-m'}`}>
                                            <TrendingUp size={11} className={asset.Leader_Count === 0 ? "opacity-30" : ""} />
                                            {asset.Leader_Count}
                                        </div>
                                    </td>
                                    <td className="px-5 py-3 text-center">
                                        <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border font-mono text-xs font-bold ${asset.Follower_Count > 0 ? 'bg-blue-500/10 border-blue-500/20 text-blue-500' : 'bg-[var(--surface-hover)] border-transparent t-text-m'}`}>
                                            <TrendingDown size={11} className={asset.Follower_Count === 0 ? "opacity-30" : ""} />
                                    ...
                                    <td className="px-5 py-3 text-right">
                                        <div className="flex items-center justify-end gap-3">
                                            <div className="flex flex-col items-end gap-0.5">
                                                <span className={`font-bold text-lg font-mono ${hasConnections ? 'text-awb-gold' : 't-text-m'}`}>{asset.Total_Relations}</span>
                                                <span className={`text-[9px] font-bold uppercase tracking-widest ${!hasConnections ? 't-text-m' : netPositive ? 'text-emerald-500' : 'text-blue-500'}`}>
                                                    {!hasConnections ? 'Isolated' : netPositive ? 'Alpha Leader' : 'Beta Follower'}
                                                </span>
                                            </div>
                                        </div>
                                    </td>
